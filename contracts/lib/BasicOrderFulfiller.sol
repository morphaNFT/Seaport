...
contract BasicOrderFulfiller is OrderValidator {
    ...
    function _validateAndFulfillBasicOrder(BasicOrderParameters calldata parameters) internal returns (bool) {
        ...
        BasicOrderRouteType route;  // 0 native token erc721  1 native token erc1155
        OrderType orderType;

        // Declare additional recipient item type to derive from the route type.
        ItemType additionalRecipientsItemType;  // eth or erc721 or erc1155 or erc20
        ...
        assembly {
            ...
            orderType := and(basicOrderType, 3)  // 按位与操作

            // Divide basicOrderType by four to derive the route.
            route := shr(2, basicOrderType)    // 右移指令
...

        {
            ...
            // 判断支付币种是否是主币和msg.value做匹配，如果是主币，msg.value必须有值，如果是token，msg.value不能有值
            // Utilize assembly to compare the route to the callvalue.
            assembly {
                // route 0 and 1 are payable, otherwise route is not payable.
                correctPayableStatus := eq(
                    additionalRecipientsItemType,        // 0 主币  > 0 token
                    iszero(callvalue())
                )
            }
        ...
        address additionalRecipientsToken; // zone
